...
function readInputs(){
  ...
  inputs.forEach(function(item){
    value = item.value;
    switch(item.id){
      case "Quint": values[item.id] = parseFloat(item.value); break;
      case "TSBoost": values[item.id] = parseFloat(item.value); break;
      case "Satchel": values[item.id] = parseFloat(item.value); break;
      case "Dropboost": values[item.id] = parseInt(item.value);
                        if(document.getElementById("globaldrop").checked){
                          values.Globaldrop = 100 + parseInt(item.value);
                        } break;
      case "tool": values.Slots[item.id] = parseInt(item.value); break;
      case "suit": values.Slots[item.id] = parseInt(item.value); break;
      case "hat": values.Slots[item.id] = parseInt(item.value); break;
      case "mitts": values.Slots[item.id] = parseInt(item.value); break;
      case "shoes": values.Slots[item.id] = parseInt(item.value); break;
      default: values[item.id] = parseInt(item.value); break;
    }
  });
  saveInCookie();
  calcs();
...
function equipImport(data){
  ...
  help = help.split(" ");
  switch(help.length){
    case 2: rarity = help[0];
            slot = help[1];
            break;
    case 3: if(/\+/.test(help[2])){
              rarity = help[0];
              slot = help[1];
...
function checkValue(itemid){
  ...
  switch(itemid){
    case "Quint": value = parseFloat(value); break;
    case "TSBoost": value = parseFloat(value); break;
    case "Satchel": value = parseFloat(value); break;
...
function calcs(){
  ...
  Object.keys(calculations).forEach(function(key){
    switch(key){
      case "DungeonOff": dungeonactive = 0; break;
      case "DungeonOn": dungeonactive = 1; break;
      case "DungeonAvg": dungeonactive = 2; break;
    }
...
function drawCalculations(){
  ...
  for(var i = 0; i < 3;i++){
    ...
    outputrow.appendChild(outputcell);outputcell = document.createElement("TH");
