...
function baseCost(amount, type) {
    ...
    switch (type) {
        case "Quint":
            basecost = quintcostperlevel;
            level = Math.round(amount / quintincrement);
            break;
        case "Resources":
            basecost = resscostperlevel;
            level = Math.round(amount / ressincrement);
            break;
        case "Satchel":
            basecost = satchelbasecostperlevel;
            level = Math.round(amount / satchelincrement);
            break;
    }
...
function ressIncomeDungeon(level, quint, ress, ressbase, tp, satchel, embellished, dungeonactive, drop, ww) {
    ...
    switch (dungeonactive) {
        case 0:
            satchel = 1 + satchel / 4 / 100;
            break;
        case 1:
            satchel = 1 + satchel / 100;
            break;
        case 2:
            satchel = 1 + (satchel / 4 + (satchel * 3 / 4 * (dungeonactionavg / avgingotchance))) / 100;
            break;
    }
...
function itemBoosts(item, stat, slotlvl) {
    ...
    switch (item.Rarity) {
        case "basic":
            raritymultiplier = basic;
            break;
        case "normal":
            raritymultiplier = normal;
            break;
        case "rare":
            raritymultiplier = rare;
            break;
        case "epic":
            raritymultiplier = epic;
            break;
        case "legendary":
            raritymultiplier = legendary;
            break;
        case "runic":
            raritymultiplier = runic;
            break;
    }
    switch (item[stat]) {
        case "Quint":
...
function xpGainedCurrentLevel(currentlvl, library, xpboost, torch, tavern, dungeonactive, drop, ww) {
    ...
    switch (dungeonactive) {
        ...
        case 1:
            ...
            break;
        case 2:
            torch = (torch / 4 + (torch * 3 / 4 * (dungeonactionavg / (ingotdropchance / (1 + drop / 100) / (1 + ww / 100)))));
            break;
    }
...
function calculateScore(slot) {
    ...
    switch (item.Rarity) {
        case "basic":
            itemRarityFactor = basic * 2;
            break;
        case "normal":
            itemRarityFactor = normal * 2;
            break;
        case "rare":
            itemRarityFactor = rare * 2;
            break;
        case "epic":
            itemRarityFactor = epic * 3;
            break;
        case "legendary":
            itemRarityFactor = legendary * 3;
            break;
        case "runic":
            itemRarityFactor = runic * 3;
            break;
    }
